...
def recognize_gesture(hand_landmark,finger_statuses):
    ...
    forth_finger_tip = hand_landmark.landmark[16]
    little_finger_tip = hand_landmark.landmark[20]
    ...
    for i in range(1,5):
        print(f"第{i+1}个手指的张开情况:{finger_statuses[i]}")
    ...
    if thumb_index_angle is not None and index_middle_angle is not None and middle_forth_angle is not None and forth_little_angle is not None:
        if thumb_index_angle < 9 and finger_statuses[2] and finger_statuses[3]:
            ...
            return "stone"
        elif (finger_statuses[1] == finger_statuses[2] == finger_statuses[3] == True) and thumb_little_angle > 20:
            return "cloth"
        ...
        for hand_landmark in results.multi_hand_landmarks:
            #for hand_landmark in results.multi_hand_landmarks:
                ...
                if isalive:
                    ...
                    print(f"第5个手指的长度是{np.linalg.norm(v)}")
